...
export default function App() {
  ...
  const calcs = useMemo(() => {
    ...
    const lines = [...areaLines, ...repairLines, ...travelLines, ...manualLines].filter((line) => line.total > 0 || line.qty > 0 || line.description);
    ...
    const materialExpense = lines.reduce((sum, line) => sum + (line.qty * (line.materialRate || 0)), 0);
    ...
    const groupedClientAreas = Object.values(areaLines.reduce((acc, line) => {
      ...
      if (!acc[key]) acc[key] = { id: `client-area-${key}`, source: "Area", description: key, details: [], qty: 1, unit: "allowance", rate: 0, materialRate: 0, total: 0, labourHours: 0 };
      acc[key].details.push(line.clientDetail || line.description);
      acc[key].total += line.total;
      acc[key].labourHours += line.labourHours || 0;
      return acc;
    }, {}));
    const otherClientLines = [...repairLines, ...manualLines].map((line) => ({ ...line }));
    const clientLines = [...groupedClientAreas, ...otherClientLines];

    return { lines, clientLines, workSubtotal, minimumAdjustment, subtotalBeforeOverhead, overheadAmount, subtotalBeforeMarkup, markupAmount, subtotal, targetMarginAdjustment, preTax, hstAmount, total, deposit, directCost, labourExpense, materialExpense, vehicleExpense, labourHours, activeHours, suggestedClockHours, factor, margin, market, marketLabel, travelTotal, materialTakeoff, compoundPlan, stagePlan, estimatedWorkDays, suggestedTripCount };
  }, [areas, repairs, travel, manual, settings]);
  ...
  function saveLocal() {
    localStorage.setItem("drywallEstimatorDraft", JSON.stringify({ project, settings, areas, repairs, travel, manual }));
  ...
  function loadLocal() {
    ...
    setRepairs(data.repairs || []); setTravel(data.travel || []); setManual(data.manual || []);
  }
  ...
  return (
    <main className={`app ${settings.darkMode === "Yes" ? "dark" : ""}`}>
      ...
      {tab === "materials" && <Card title="Material Takeoff"><div className="help">This is an estimate only. It uses your area lines, waste percentage, sheet size, and rough coverage assumptions from Settings.</div><MaterialTakeoff calcs={calcs} settings={settings} /></Card>}
...
function Totals({ calcs, showInternal = false }) {
  ...
  return <div className="totals">{rows.filter(([label, value]) => alwaysShow.includes(label) || value !== 0).map(([label, value]) => <div className={label === "Total" ? "totalLine big" : "totalLine"} key={label}><span>{label}</span><b>{money(value)}</b></div>)}<div className="miniStats miniStats4"><span>Labour expense: {money(calcs.labourExpense)}</span><span>Material expense: {money(calcs.materialExpense)}</span><span>Vehicle expense: {money(calcs.vehicleExpense)}</span><span>Margin: {fmt(calcs.margin * 100, 1)}%</span><span>Direct cost: {money(calcs.directCost)}</span><span>Crew efficiency: {fmt(calcs.factor * 100, 1)}%</span></div></div>
}
...
function MaterialTakeoff({ calcs, settings }) { const m = calcs.materialTakeoff; return <div className="takeoffGrid"><div className="takeoffCard"><h3>Total board area</h3><b>{fmt(m.totalArea)} sq ft</b><small>Before waste</small></div><div className="takeoffCard"><h3>Drywall sheets</h3><b>{Math.ceil(m.sheets)}</b><small>Includes {fmt(settings.wastePercent,0)}% waste</small></div><div className="takeoffCard"><h3>Compound boxes/bags</h3><b>{Math.ceil(m.mudBoxes)}</b><small>Rough estimate based on finish coats</small></div><div className="takeoffCard"><h3>Tape rolls</h3><b>{Math.ceil(m.tapeRolls)}</b><small>{fmt(m.linearTapeFt,0)} linear ft estimated, {settings.tapeCoveragePerRoll} ft/roll</small></div><div className="takeoffCard"><h3>Screws</h3><b>{Math.ceil(m.screws)}</b><small>{settings.screwsPerSheet} screws/sheet</small></div><div className="takeoffCard"><h3>Board breakdown</h3>{Object.entries(m.byBoard).length ? Object.entries(m.byBoard).map(([k,v]) => <p key={k}>{k}: <b>{Math.ceil(v)}</b> sheets</p>) : <small>No board needed yet.</small>}</div></div> }
