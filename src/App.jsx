...
const materialLibrary = {
  "Custom material": { category: "Misc", unit: "each", unitCost: 0, custom: true },
...
export default function App() {
  ...
  const calcs = useMemo(() => {
    ...
    const manualMaterialLines = manualMaterials.filter((m) => m.include === "Yes").map((m) => {
      const libraryItem = materialLibrary[m.material] || materialLibrary["Custom material"];
      const qty = Number(m.qty) || 0;
      const unitCost = libraryItem.custom ? (Number(m.unitCost) || 0) : (Number(libraryItem.unitCost) || 0);
      const unit = libraryItem.unit || m.unit || "each";
      const category = libraryItem.category || m.category || "Misc";
  ...
  const updateManualMaterial = (id, field, value) => setManualMaterials((rows) => rows.map((r) => {
    ...
    if (field === "material") {
      const item = materialLibrary[value] || materialLibrary["Custom material"];
      next.category = item.category || "Misc";
      next.unit = item.unit || "each";
      next.unitCost = Number(item.unitCost) || 0;
    }
    if (field === "unitCost") next.unitCost = Number(value) || 0;
    ...
    <div className="manualBox">
      ...
      <div className="tableWrap"><table><thead><tr><th>Include</th><th>Material</th><th>Category</th><th>Qty</th><th>Price</th><th>Waste</th><th>Total</th><th>Notes</th><th></th></tr></thead><tbody>
        {manualMaterials.length === 0 ? <tr><td colSpan="9"><Empty text="No manual materials added yet." /></td></tr> : manualMaterials.map((mat) => {
          ...
          const unit = libraryItem.unit || mat.unit || "each";
          const category = libraryItem.category || mat.category || "Misc";
          const total = (Number(mat.qty) || 0) * unitCost * (1 + ((Number(settings.wastePercent) || 0) / 100));
          return <tr key={mat.id}>
            ...
            <td>{category}</td>
            <td><input type="number" value={mat.qty} onChange={(e) => updateManualMaterial(mat.id, "qty", e.target.value)} /><small className="unitHint">{unit}</small></td>
            <td>{libraryItem.custom ? <input type="number" value={mat.unitCost} onChange={(e) => updateManualMaterial(mat.id, "unitCost", e.target.value)} /> : <b>{money(unitCost)}</b>}</td>
