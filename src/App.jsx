...
export default function App() {
  ...
  const calcs = useMemo(() => {
    ...
    const manualMaterialLines = manualMaterials.filter((m) => m.include === "Yes").map((m) => {
      ...
      const wasteRate = (Number(m.waste) || 0) / 100;
    ...
    const manualMaterialEstimateLines = manualMaterialLines.map((m) => ({
      ...
      qty: m.qty,
      unit: m.unit || "each",
      rate: m.unitCost * (1 + (Number(m.waste) || 0) / 100),
      materialRate: 0,
      activeHours: 0,
      labourHours: 0,
      visits: 0,
    ...
    const otherClientLines = [...repairLines, ...manualLines, ...manualMaterialEstimateLines.filter((line) => line.clientVisible === "Yes")].map((line) => ({ ...line }));
...
function MaterialTakeoff({ calcs, settings, manualMaterials, updateManualMaterial, setManualMaterials }) {
  ...
  return <div className="materialsStack">
    ...
    <div className="manualBox">
      ...
      <p className="sectionNote">Use this for repair materials or trade-specific items that the automatic takeoff does not catch, like Sheetrock 45, corner bead, backing wood, sandpaper, primer, disposal bags, or specialty board.</p>
      {manualMaterials.length === 0 ? <Empty text="No manual materials added yet." /> :
      <div className="tableWrap"><table><thead><tr><th>Include</th><th>Category</th><th>Material</th><th>Qty</th><th>Unit</th><th>Unit Cost</th><th>Waste %</th><th>Client Visible?</th><th>Notes</th><th>Total</th><th></th></tr></thead><tbody>{manualMaterials.map((mat) => {
        const qty = Number(mat.qty) || 0;
        const unitCost = Number(mat.unitCost) || 0;
        const waste = (Number(mat.waste) || 0) / 100;
        ...
        return <tr key={mat.id}>
          ...
          <td><input type="number" value={mat.waste} onChange={(e) => updateManualMaterial(mat.id, "waste", e.target.value)} /></td>
          <td><ToggleYesNo value={mat.clientVisible} onChange={(v) => updateManualMaterial(mat.id, "clientVisible", v)} /></td>
